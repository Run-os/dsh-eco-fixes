/**
 * dsh-eco-fixes — DeepSeek Harness 常用插件问题修复扩展(web profile 本地插件)。
 *
 * 解决的问题:
 ...
 *   4) 显示环境(Xvfb):共享浏览器自托管窗口需要 X 显示。勾选
 *      dshBuiltinBrowserAdapt(与 electron/沙箱环境合并)后,幂等注入 DISPLAY
 *      到启动脚本、生成并启用 xvfb systemd 单元;检测项含
 *      DISPLAY 环境变量 / X socket / Xvfb 可执行文件 / 单元状态。修复对
 *      运行中进程无效,状态里会标出「需重启 dsh-web 生效」。
 *
 * 所有功能默认**全部关闭**:使用者需自行在「设置 → 插件 → 插件配置」的
 * 「常用插件扩展(dsh-eco-fixes)」菜单里勾选,勾选了的才会运行。
 * 菜单项与 ~/.dsh/dsh-eco-fixes.json 的 features 双向同步(4 项,默认全关):
 *   - autoMemoryPatch           dsh-auto-memory 公网访问补丁(默认关闭)
 *   - dshBuiltinBrowserAdapt    dsh-builtin-browser 自动适配(默认关闭;v0.5.0 起
 *                               合并了原 electronAutoInstall / runScriptSandboxEnv /
 *                               displayEnv 三个勾选,一勾全做:
 *                                 · electron 二进制缺失时后台 npx install-electron
 *                                 · run-dsh-web.sh 注入 ELECTRON_DISABLE_SANDBOX
 *                                 · 显示环境:Xvfb 单元 + 注入 DISPLAY)
 *   - sidebarFooterStack        左侧边栏底部按钮纵向排列(默认关闭,client 端生效)
 *   - pluginMenuStyleAdapter    插件菜单未样式卡片自动补外观(默认关闭,client 端生效)
 *
 * 接口:
 *   - 每次启动自动体检修复,无需手工操作。
...
/** features 默认勾选:全部默认关闭,由使用者在「设置 → 插件」菜单里自行勾选,
 *  勾选了的才会运行。旧配置(无 features 字段)按 resolveFeatures 折算保持原行为。 */
export function defaultFeatures() {
  return {
    // dsh-auto-memory 公网访问补丁,默认关闭
    autoMemoryPatch: false,
    // dsh-builtin-browser 自动适配:electron 自动安装 + 启动脚本沙箱注入 + 显示环境
    // (v0.5.0 起三者合并为一个勾选项;历史配置的旧键自动折算,见 resolveFeatures)
    dshBuiltinBrowserAdapt: false,
    // 左侧边栏底部按钮纵向排列,默认关闭(client 端生效)
    sidebarFooterStack: false,
    // 插件菜单样式自动适配默认关闭(纯 client 端 CSS 适配,不修改任何插件文件)
...
export function defaultConfig() {
  return {
    ...
    // 显示环境(共享浏览器自托管窗口需要 X):勾选 dshBuiltinBrowserAdapt 后,
    // 本插件会幂等注入 DISPLAY 到启动脚本、并生成/启用对应的 Xvfb systemd 单元。
    display: { value: ':99', unit: 'xvfb-dsh.service', screen: '1920x1080x24' },
    // 设置-插件菜单勾选项:只有勾选的功能才会运行(默认全部关闭,由使用者
    // 在 设置 → 插件 → 插件配置 的「常用插件扩展(dsh-eco-fixes)」卡片里勾选,
    // 勾选变更会同步回本配置文件)。
 ...
 * 保证旧配置文件升级后行为不变。结果写回 cfg.features。
 *
 * v0.5.0 起 electronAutoInstall / runScriptSandboxEnv / displayEnv 三个旧勾选
 * 合并为 dshBuiltinBrowserAdapt:旧配置里三者任一为 true,合并项即视为开启;
 * 显式写了新键则以新键为准。
 */
export function resolveFeatures(cfg) {
  const legacy = defaultFeatures()
  legacy.autoMemoryPatch = cfg.autoMemory.enabled !== false
  const mergedFromLegacyCfg = (cfg.browser.electron.autoInstall !== false) || (cfg.browser.electron.disableSandboxEnv !== false)
  legacy.dshBuiltinBrowserAdapt = mergedFromLegacyCfg
  const f = { ...legacy }
  if (cfg.features && typeof cfg.features === 'object') {
    // 旧版 6 键 features:先把三个子项折算成合并项(任一勾选即开启)
    const hasOldKeys = typeof cfg.features.electronAutoInstall === 'boolean'
      || typeof cfg.features.runScriptSandboxEnv === 'boolean'
      || typeof cfg.features.displayEnv === 'boolean'
    if (hasOldKeys) {
      f.dshBuiltinBrowserAdapt = cfg.features.electronAutoInstall === true
        || cfg.features.runScriptSandboxEnv === true
        || cfg.features.displayEnv === true
    }
...
export function kickElectronInstall(cfg) {
  ...
  if (cfg.features && cfg.features.dshBuiltinBrowserAdapt === false) {
    return { kicked: false, reason: 'dshBuiltinBrowserAdapt 未勾选(设置-插件),跳过' }
  }
...
export function patchRunScript(cfg) {
  if (cfg.features && cfg.features.dshBuiltinBrowserAdapt === false) {
    return { ok: true, changed: false, skipped: 'dshBuiltinBrowserAdapt 未勾选(设置-插件),跳过' }
  }
...
/**
 * 修复显示环境(仅在勾选 dshBuiltinBrowserAdapt 时运行;幂等):
...
export function currentStatus(cfg) {
  ...
  const restartNeeded = state.electronInstalledThisSession || runScript.changed
    ...
    // dsh-builtin-browser 自动适配已勾选但当前进程 DISPLAY 未指向目标 display → 需重启继承
    || (cfg.features && cfg.features.dshBuiltinBrowserAdapt === true && !display.envMatches)
    || false
  return {
    ...
    display: {
      ok: display.ok,
      enabled: !!(cfg.features && cfg.features.dshBuiltinBrowserAdapt),
  ...
  ctx.inject(['settings'], (sctx) => {
    try {
      ...
      const scope = sctx.settings.register(SETTINGS_NS, z.object({
        ...
        dshBuiltinBrowserAdapt: z.boolean().default(true),
        sidebarFooterStack: z.boolean().default(false),
        pluginMenuStyleAdapter: z.boolean().default(false),
      }), { base: { ...cfg.features } })
      menuScope = scope
      ...
      scope.watch(() => {
        try {
          ...
          cfg.features = {
            ...
            dshBuiltinBrowserAdapt: v.dshBuiltinBrowserAdapt !== false,
            sidebarFooterStack: v.sidebarFooterStack === true,
            pluginMenuStyleAdapter: v.pluginMenuStyleAdapter === true,
          }
          persistFeatures(cfg)
      ...
      if (llm !== undefined) {
        try {
          llm.registerConfigurableProviders([{
            ...
            displayName: '常用插件扩展（dsh-eco-fixes）',
...
export function apply(ctx, config) {
  ...
  // v0.5.0 配置迁移:旧版 6 键 features 已在内存折算为 4 键,顺带把文件落成新格式
  try {
    const rawFile = fs.existsSync(CONFIG_PATH) ? JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8')) || {} : {}
    const rawFeats = (rawFile && rawFile.features) || {}
    if (typeof rawFeats.electronAutoInstall === 'boolean'
      || typeof rawFeats.runScriptSandboxEnv === 'boolean'
      || typeof rawFeats.displayEnv === 'boolean') {
      persistFeatures(cfg)
      log('已把旧版 features(6 键)迁移为新版 4 键(合并为 dshBuiltinBrowserAdapt=' + cfg.features.dshBuiltinBrowserAdapt + ')')
    }
  } catch (e) { /* 迁移失败不影响启动 */ }
  try {
