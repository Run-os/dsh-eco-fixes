 ...
 * 菜单项与 ~/.dsh/dsh-eco-fixes.json 的 features 双向同步(4 项):
 *   - autoMemoryPatch           自动记忆(dsh-auto-memory)白名单补丁自动重打
 *   - dshBuiltinBrowserAdapt    dsh-builtin-browser 自动适配(v0.5.0 起合并了原
 *                               electronAutoInstall / runScriptSandboxEnv /
...
/** features 的默认勾选:autoMemoryPatch 默认开;dsh-builtin-browser 自动适配默认
 *  开(覆盖 electron 安装/沙箱环境/显示环境三件事);footerStack/样式适配默认关。 */
export function defaultFeatures() {
  return {
    ...
    dshBuiltinBrowserAdapt: true,
    // 侧边栏底部按钮各占一行,默认关闭(client 端生效)
