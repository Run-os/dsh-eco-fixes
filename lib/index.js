...
/** features 的默认勾选(保持历史行为:前三个默认开,footerStack 默认关)。 */
...
export function currentStatus(cfg) {
  ...
  const restartNeeded = state.electronInstalledThisSession || runScript.changed || (electron.present && !electron.binaryOk && state.electronInstalling) || false
  return {
...
function runFixes(cfg, log, hint) {
  ...
  if (hint) {
    ...
    log(hint + ': autoMemory=' + memTxt + ' | electron=' + elecTxt + ' | runScript=' + runTxt)
  }
  return { mem, elec, runScr }
}
    ...
    const routes = [
      ...
      },
      {
        ...
        handler: (req, res) => {
          ...
          st.lastApply = { autoMemory: mem, runScript: runScr, electronKick: kick, at: new Date().toISOString() }
          sendJson(res, 200, st)
        },
      },
    ]
...
export function apply(ctx, config) {
  ...
  try {
    ...
    const { mem, elec, runScr } = runFixes(cfg, log)
    ...
    log('boot 体检完成: autoMemory=' + memTxt
      ...
      + ((state.electronInstalledThisSession || runScr.changed) ? ' | 提示:需重启 dsh-web 使浏览器生效' : ''))
  } catch (e) {
